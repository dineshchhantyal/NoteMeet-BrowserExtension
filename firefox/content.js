// Update browser definition at the top of content.js
...
async function handleAuth() {
    try {
        ...
        logError("Error in handleAuth:", error);
    }
}
...
function floatingWindow() {
  ...
  // Define these variables in the scope where they're needed
  function showPanel() {
...
browser.runtime.onMessage.addListener((message, sender, sendResponse) => {
  if (message.type === "AUTH_STATE_CHANGED") {
    handleAuthStateChange(message.user);
    // Firefox requires explicit return true for async responses
...
async function startRecording() {
  try {
    // Firefox requires more explicit media constraints
    screenStream = await navigator.mediaDevices.getDisplayMedia({
      video: {
        mediaSource: "screen",
        width: { max: 1920 },
    ...
    // Firefox handles audio differently
    micStream = await navigator.mediaDevices.getUserMedia({
      audio: {
        echoCancellation: true,
        noiseSuppression: true,
    ...
    logError('Error during screen recording:', error);
    throw error;
  }
}

// Update file URL creation
...
async function getStoredData(key) {
  try {
    ...
    logError("Error accessing storage:", error);
    return null;
  }
}
...
async function setStoredData(key, value) {
  try {
    ...
    logError("Error setting storage:", error);
  }
}
